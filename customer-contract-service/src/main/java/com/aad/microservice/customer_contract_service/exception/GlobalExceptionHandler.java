...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleAppException(AppException ex) {
        ...
        if (ex.getCode() == ErrorCode.NotFound_Exception ||
            ex.getCode() == ErrorCode.CustomerNotFound_Exception ||
            ex.getCode() == ErrorCode.JobCategoryNotFound_Exception) {
            status = HttpStatus.NOT_FOUND;
        } else if (ex.getCode() == ErrorCode.Duplicated_Exception) {
            status = HttpStatus.BAD_REQUEST;
        } else if (ex.getCode() == ErrorCode.NotAllowCreate_Exception ||
                   ex.getCode() == ErrorCode.NotAllowUpdate_Exception ||
                   ex.getCode() == ErrorCode.NotAllowDelete_Exception ||
                   ex.getCode() == ErrorCode.InvalidDate_Exception ||
                   ex.getCode() == ErrorCode.InvalidInput_Exception) {
            status = HttpStatus.BAD_REQUEST;
        }
